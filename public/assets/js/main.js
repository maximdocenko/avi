...
*/
        ...
        case 'client-ready-oncall':
            displayAlert(username, profile, 'Is on another call');
            setTimeout('window.location.reload(ture)', 2000);
        break;

        case 'client-rejected':
            displayAlert(username, profile, 'is busy');
        break;

        case 'client-hangup':
            displayAlert(username, profile, 'Disconnected the call');
            setTimeout('window.location.reload(ture)', 2000);
        break;
    }
}
...
function redirectToCall(username, sendTo) {
    if(window.location.href.indexOf(username) == -1) {
        ...
        window.location.href = '/chat/home/'+username;
    }
}
